...
The remainder of the paper is organized as follows. Section II reviews related work on educational early warning systems, academic performance prediction, explainable analytics, and retrieval-grounded advisory systems. Section III describes the SARS architecture and methodology. Section IV formalizes the risk scoring model. Section V details the evidence-grounded advisory pipeline. Section VI presents evaluation results. Section VII discusses practical implications and limitations. Section VIII concludes the paper and outlines future work.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\columnwidth]{system_architecture_diagram.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.98\columnwidth]{extraction_review.jpeg}
...
\section{Evidence-Grounded Advisory Pipeline}
\subsection{Motivation for Selective Retrieval}
...
\subsection{Retrieval and Response Workflow}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\columnwidth]{rag_pipeline_diagram.pdf}
...
\subsection{Automation Behaviors}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.98\columnwidth]{risk_monitor.jpeg}
...
Several limitations remain. SQLite is convenient for a prototype and small deployments but is not appropriate for heavily concurrent institutional production use. The retrieval-based advisory pipeline depends on Gemini API availability, and free-tier or quota-based rate limits can constrain usage at scale. The thresholds used in the current risk engine, including the 7.5 CGPA placement cut-off and 75\% attendance expectation, are calibrated for a specific institutional context and must be recalibrated before transfer to other universities. In addition, multilingual advisory support has not yet been implemented, and longitudinal validation across multiple cohorts and institutions has not been completed. These observations align with broader cautions in educational analytics and ML deployment research \cite{hussain2019predicting,summers2021predicting}.
